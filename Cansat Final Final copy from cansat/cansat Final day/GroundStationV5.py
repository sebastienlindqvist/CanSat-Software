...
while True:
    ...
    data_recieved_file.close()
    data_recieved.split(", ")
    #data=[Accel_X,Accel_Y,Accel_Z,Mag_X,Mag_Y,Mag_Z,Altt_Value,Long_Value,Latt_Value,Num_Sat,gps_vel,gps_heading,gps_dilusion]
    Accel_X=data_recieved[0]
    Accel_Y=data_recieved[1]
    Accel_Z=data_recieved[2]
    Mag_X=data_recieved[3]
    Mag_Y=data_recieved[4]
    Altt_Value=data_recieved[5]
    Long_Value=data_recieved[6]
    Latt_Value=data_recieved[7]
    Num_Sat=data_recieved[8]
    gps_vel=data_recieved[9]
    gps_heading=data_recieved[10]
    gps_dilusion=data_recieved[11]
    TimeItWasReceived=data_recieved[12]
    ...
    #except:
    #    pass
    #Eta_num=Altt_Value/Current_Vel_Y
    #Horizontal_Vel=math.sqrt((Current_Vel_X^2)+(Current_Vel_Z^2))
    #Abs_Vel=math.sqrt((Current_Vel_X^2)+(Current_Vel_Y^2)+(Current_Vel_Z^2))



    data=[Current_Glider_R_Pos,Current_Glider_L_Pos]
    data_sent=', '.join([str(elem) for elem in data])
    data_to_send=open("Data_to_send.txt","w")
    data_to_send.write(data_sent)
    data_to_send.close()

    #Update_map()
    #Update_Arrow_Compass()